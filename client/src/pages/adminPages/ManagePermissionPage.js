...
const ManagePermissionPage = () => {
  ...
  const fetchUserRoles = useCallback(
    async (currentPage, currentSize, selectedRole) => {
      setIsLoading(true);
      setError(null);
      try {
        const data = await adminPermissonApi.getUserRoleList(
          currentPage,
          currentSize,
          selectedRole,
        );
  ...
  const handleSubmitChange = async () => {
    ...
    try {
      ...
      const data = await adminPermissonApi.updateUserRole(
        newRole,
        selectedUserMap,
      );

      alert(
        `${data.updatedUserCount}명의 사용자가 ${data.changeTo} 권한으로 변경되었습니다.`,
      );
                  ...
                  {users.length > 0 ? (
                    users.map((user) => (
                      <tr
                        ...
                        className={
                          selectedUserMap.has(user.id) ? styles.selectedRow : ""
                        }
      ...
      <div className={styles.Box}>
        ...
        <div className={styles.changeContent}>
          ...
          <div className={styles.controler}>
            <span
              style={{ fontSize: "18px", fontWeight: "700", minWidth: "77px" }}
            >
              다음으로 권한 변경:
            </span>
